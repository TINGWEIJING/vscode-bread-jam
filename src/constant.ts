...
/**
 * Example of a render pattern:
 * `01 Subtext - Fade In Gradient - Unique Subtext`
 */
export const ALL_RENDER_PATTERN_SET = new Set(
  RENDER_PATTERN_LABEL.map(
    (value, index) => `${String(index + 1).padStart(2, "0")} ${value}`,
  ),
);
...
export const DEFAULT_SELECTED_RENDER_PATTERN = `${QUICK_PICK_ITEMS[1].label} ${QUICK_PICK_ITEMS[1].description}`;
